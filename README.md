...
`assets/sprites/` 内に PNG が無いため programmatic フォールバックで描画されるが、機能リグレッションなしで v1-0a 同等の見た目を維持する。**加えて壁から床への hex 形状 drop shadow が描画され、立体感が増す**。アセット投入は v1-0b.2 で実施。

ドキュメント側(CHANGELOG.md / STATUS.md / NEXT_STEPS.md)の更新分は `docs/snippets/` 内のスニペットを既存ドキュメントに手動マージすること。

## ファイル構成(全 30 ファイル)

### ソースコード(ルート直下、16 ファイル)
...
- `render.js.diff`(395 行) — unified diff(v1-0a → v1-0b.1)
- `game.js.diff`(32 行)
...
- 全 15 JS ファイル `node --check` syntax OK
- 全 import-export 整合性 OK
...
- **v1-0b.3**: shadow `angleDeg` / `lengthRatio` / `alpha` の実機チューニング、`ctx.filter` 数値チューニング、HiDPI 検証、必要なら offscreen canvas キャッシュ実装、rooms_classic family の wall 明示登録 refactor
